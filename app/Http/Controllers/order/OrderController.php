...
class OrderController extends Controller
{
    ...
    public function load_header(Request $request)
    {
        ...
        if ($store === 'ALL') {
            $nota = Sale::selectRaw('count(DISTINCT id_invoice) as id_invoice')->where('customer', 'RETAIL')->where('payment', 'PAID')->whereBetween('tanggal', [$start, $end])->get('id_invoice');
            $qty = Sale::whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->sum('qty');
            $ongkir = Sale::selectRaw('ongkir')->whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->groupBy('id_invoice')->get();
            $gross_sale = Sale::whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->get();
            $expenses = Store_equipment_cost::whereBetween('tanggal', [$start, $end])->sum('total_price');
            $discount_item = Sale::whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->sum('diskon_item');
            $discount_all = Sale::selectRaw('diskon_all')->whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->groupBy('id_invoice')->get('diskon_all');
        } else {
            $nota = Sale::selectRaw('count(DISTINCT id_invoice) as id_invoice')->where('id_store', $store)->whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->get('id_invoice');
            $qty = Sale::whereBetween('tanggal', [$start, $end])->where('id_store', $store)->where('customer', 'RETAIL')->where('payment', 'PAID')->sum('qty');
            $ongkir = Sale::selectRaw('ongkir')->where('id_store', $store)->whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->groupBy('id_invoice')->get();
            $gross_sale = Sale::whereBetween('tanggal', [$start, $end])->where('id_store', $store)->where('customer', 'RETAIL')->where('payment', 'PAID')->get();
            $expenses = Store_equipment_cost::whereBetween('tanggal', [$start, $end])->where('store', $store)->sum('total_price');
            $discount_item = Sale::whereBetween('tanggal', [$start, $end])->where('id_store', $store)->where('customer', 'RETAIL')->where('payment', 'PAID')->sum('diskon_item');
            $discount_all = Sale::selectRaw('diskon_all')->where('id_store', $store)->whereBetween('tanggal', [$start, $end])->where('customer', 'RETAIL')->where('payment', 'PAID')->groupBy('id_invoice')->get('diskon_all');
        }
            ...
            if ($last_id == '0') {
                if ($querys_result == '') {
                    ...
                    $data = Sale::with('details', 'store', 'reseller')
                        ...
                        ->where([['id_invoice', $querys_result], ['customer', 'RETAIL']])
                        ->orwhere([['produk',  'Like', '%' . $querys_result . '%'], ['customer', 'RETAIL']])
                        ->orwhere([['id_produk',  $querys_result], ['customer', 'RETAIL']])
                        ->orwhere([['users', $querys_result], ['customer', 'RETAIL']])
                ...
                if ($querys_result == '') {
                    ...
                    $data = Sale::with('details', 'store', 'reseller')
                        ->selectRaw('*,GROUP_CONCAT(produk SEPARATOR " ") as produk,GROUP_CONCAT(id_produk SEPARATOR " ") as id_produk')
                        ->where([['id_invoice', '<', $last_id], ['produk',  'Like', '%' . $querys_result . '%'], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['id_produk', $querys_result], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['id_invoice', $querys_result], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['users', $querys_result], ['customer', 'RETAIL']])
            ...
            if ($last_id == '0') {
                if ($querys_result == '') {
                    ...
                    $data = Sale::with('details', 'store', 'reseller')
                        ...
                        ->where([['id_invoice', $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['produk',  'Like', '%' . $querys_result . '%'], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['id_produk',  $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['users', $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
                ...
                if ($querys_result == '') {
                    ...
                    $data = Sale::with('details', 'store', 'reseller')
                        ...
                        // ->where('customer', 'RETAIL')
                        // ->where('id_store', $store)
                        // ->where([['id_invoice', '<', $last_id], ['id_reseller', $querys_result], ['customer', 'RETAIL']])
                        ->where([['id_invoice', '<', $last_id], ['produk',  'Like', '%' . $querys_result . '%'], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['id_produk', $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['id_invoice', $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
                        ->orwhere([['id_invoice', '<', $last_id], ['users', $querys_result], ['id_store', $store], ['customer', 'RETAIL']])
    ...
    // public function print_sales_retail(Request $request, $id_invoice)
    // {
    //     $details = Sale::with('store_inv')->selectRaw('*,SUM(qty) as qty, SUM(subtotal) as subtotal')->where('id_invoice', $id_invoice)->groupBy('id_invoice', 'id_produk')->get();
